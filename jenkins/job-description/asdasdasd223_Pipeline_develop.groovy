...
def job_config = [
        job: [
                name: "asdasdasd223_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asdasdasd223_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdasdasd223_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdasdasd223.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdasdasd223", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdasdasd223_Pipeline_develop");

job.with {

    authenticationToken('Jznk68rtrcYGJQpXJTTbMA6VWYfgCCbX')
